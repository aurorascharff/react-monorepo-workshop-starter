...
1. In the dashboard, remove state and effects that duplicate information already available from patient data. Calculate the totals during render instead.

2. In the patient list, keep state for the user's filter choices, but derive the filtered patients from existing data. The search and gender filter should keep working.
